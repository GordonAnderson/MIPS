...
void CreateFile(char *FileName)
{
  ...
  if(strlen(FileName) > 20) ERR(ERR_FILENAMETOOLONG);
  if(SD.exists(FileName)) ERR(ERR_CANTCREATEFILE);
  // Open the file
  if(!(file = SD.open(FileName, FILE_WRITE))) ERR(ERR_CANTOPENFILE);
  // Write comment to file and close
  file.print("# Created file: ");
...
void AddFile(void)
{
  ...
  while(true)
  {
     // Read the file name and open file
     if((tkn = TokenFromCommandLine(',')) == NULL) break;
     SD.begin(_sdcs);
     if(!SD.exists(tkn)) ERR(ERR_CANTOPENFILE);
     if(!(file = SD.open(tkn, FILE_WRITE))) ERR(ERR_CANTOPENFILE);
     // Read from ring buffer and write to file until end of line
     RB_Get(&RB);
     while(true)
     {
       c = RB_Get(&RB);
...
void MoreFile(char *FileName)
{
  File  file;
  char  c;

  SD.begin(_sdcs);
  if(!SD.exists(FileName)) ERR(ERR_CANTOPENFILE);
  if(!(file = SD.open(FileName, FILE_READ))) ERR(ERR_CANTOPENFILE);
...
void ReadLine(char *FileName, char *line)
{
  String Token;
  File  file;
  char  c;
  ...
  {
    c = file.read();
    if(i == il) serial->write(c);
    if(c == '\n') i++;
    if(i > il) break;
  }
  if(il > i) serial->println("EOL");
  file.close();
  SendACK;
