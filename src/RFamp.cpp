...
//
// Updates for rev 3.0 module: (April 2026)
//   This module uses 18bit DACs for setting the drive level and the RF level setpoint. The CPLD logic was updated
//   to generate the needed chip selects to support the SPI 18bit DACs. The 3.0 modules also supports the QUAD resolving
//   DC module. The DC module also uses 18bit DACs using chip slects from the rev 3.0 quad module. This driver support
...
//    4.) Add scanning function. Scan m/z and store data in internal buffer. Read data from selected
//        ADC port.
//        - Commands for the following:
//          - Start m/z
//          - Stop m/z
//          - Scan rate, m/z per sec
//          - Number of samples
//          - ADC channel for detector
//          - Average mode
//          - Trigger, external
//          - Start command
//          - Abort
...
//          - Satus command
//    5.) Add mass calibration procedure, currently have voltage level calibration. This function
//        will support accurate scanning of m/z range
...
// Calibration read functions (defined later in RFamp.cpp)
void RFdrvCalPrep(void);
void RFpP1readLR(void);
void RFpP2readLR(void);
void RFnP1readLR(void);
void RFnP2readLR(void);
void RFspP1readLR(void);
void RFspP2readLR(void);
void RFpP1read(void);
void RFpP2read(void);
void RFnP1read(void);
void RFnP2read(void);
void RFspP1read(void);
void RFspP2read(void);
...
// Delay before the sweep starts, and after the drive level is changed, in 100mS units.
// This one is NOT a filter artifact: it has to cover the RF level actually decaying.
// If the module was already tuned and driving a high output, the level at the start
// frequency reads high for a while after the frequency jumps off resonance. Max is set
// from that stale reading, nothing later beats it, and the tune converges on the start
// frequency. 4 seconds; increase if a tune started from a high output state still fails.
int  quadATstartDelay = 40;
...
const Commands  RFAMPCmdArray[] = {
// Start of command block
//
// General RFamp and QUAD commands
//
  ...
  {"GRFAVPPB", CMDfunction, 1, (char *)RFAgetVPPB},          // Returns the RF output B actual level
  {"GFRAVPPB", CMDfunction, 1, (char *)RFAgetVPPB},          // Legacy misspelling of GRFAVPPB, retained for
                                                            // compatibility. Use GRFAVPPB in new host code.
  ...
  {"RFAACQ", CMDfunctionLine, 1, (char *)RFAacquire},        // Report last point and acquire,module,mz,delta,dwell
...
// RF amp / quad auto tune
  ...
  {"SRFATMINF", CMDint, 1, (char *)&quadATminF},             // Set auto tune start frequency
  {"GRFATMINF", CMDint, 0, (char *)&quadATminF},             // Return auto tune start frequency
  {"SRFATMAXF", CMDint, 1, (char *)&quadATmaxF},             // Set auto tune stop frequency
  ...
  {"SRFATSTARTDLY", CMDint, 1, (char *)&quadATstartDelay},    // Set auto tune sweep start delay, 100mS units
  {"GRFATSTARTDLY", CMDint, 0, (char *)&quadATstartDelay},    // Return auto tune sweep start delay, 100mS units
  {"SRFATFINEDLY", CMDint, 1, (char *)&quadATfineDelay},      // Set auto tune fine pass per step delay, 100mS units
...
// Configutation commands
  ...
  {"CALRFAPS", CMDfunction, 1, (char *)calRFApoleSupplies},  // Calibrates the Rev 3 pole power supplies
// End of table marker
  {0},
};
...
void setRFADAC(int brd, int channel, float value, float gc)
{
   ...
   {
      // Only DRIVE and SETPOINT are routed to 18 bit DACs on rev 3. AUX1 and AUX2
      // are not: passing them through would place their channel numbers (2 and 3)
      // on the CPLD shift register, toggling RFAcpldCLOSED and RFAcpldRANGE.
      int ch;
      if(channel == RFAdacDRIVE)         ch = RFAcpldCS_RFD;
      else if(channel == RFAdacSETPOINT) ch = RFAcpldCS_RFSP;
      else
      {
      ...
      {
         Set_18bitDAC(brd,ch,Value2Counts(value,&RFAarray[brd]->DACchansLR,gc,RFAdac18BITMAX));
         return;
      }
      Set_18bitDAC(brd,ch,Value2Counts(value,&RFAarray[brd]->DACchans[channel],gc,RFAdac18BITMAX));
      return;
   }
   if((channel == RFAdacSETPOINT) && ((RFACPLDimage[brd] & (1<<RFAcpldRANGE))) == 0)
   {
...
// This DAC is an AD5680, 18 bit, 0 to 5V output.
//
// CS_x asserts on (BRDSEL AND address match AND latched enable bit) per the CPLD
// equations. It is NOT gated by the SPI CS line, so the enable bit is armed only for
// the duration of this transaction and cleared on the way out. RFACPLDimage[] must
// never carry these bits persistently.
//
// SetAddress(0) is what raises SYNC and is therefore the AD5680 output update edge;
// the part has no LDAC pin. It must happen before the enable bit is cleared.
void Set_18bitDAC(int brd, int DACchannel, int value)
{
  AtomicBlock< Atomic_RestoreState > a_Block;

  // An out of range count shifts into the AD5680 PD1:PD0 bits, which powers the
  // output down (value > max) or three-states it (value < 0), rather than simply
  // producing a wrong voltage. Clamp hard.
  if(value < 0)                value = 0;
  if(value > RFAdac18BITMAX)   value = RFAdac18BITMAX;

  // Arm the chip select enable for this DAC
  SendFRAcpldCommand(RFAarray[brd]->CPLDspi, RFACPLDimage[brd] | (1 << DACchannel));
  // Assert CS by placing the decoded address on the bus
  ...
  {
    // Not a valid 18 bit DAC select, undo the enable and exit without clocking data
    SendFRAcpldCommand(RFAarray[brd]->CPLDspi, RFACPLDimage[brd] & ~(1 << DACchannel));
    return;
  }
  // Send Data. AD5680 clocks data on the falling edge, SPI mode 2. Set the mode and
  // divider on every call; SendFRAcpldCommand, SetDDSfrequency and AD5668 all
  // reconfigure SPI in between.
...
// This function sets the values in the data structures that will flag the processing loops
// to perform the updates. The hardware updates could be delayed up to 100 mS.
void QUADupdate(int brd)
{
  ...
  float delta = ((RFAarray[brd]->mz - RFAarray[brd]->Res) / RFAarray[brd]->mz);
  if(delta > 0) DCV *= delta;
  RFAarray[brd]->ResolvingDC = DCV;  
}

// Called when user selects mode (open or closed) number and/or the UI updates
...
void RestoreRFAsettings(bool NoDisplay)
{
  ...
  if(ReadEEPROM(&rfa_data, RFAarray[b]->EEPROMadr, 0, sizeof(RFAdata)) == 0)
  {
     if(strcmp(rfa_data.Name,RFAarray[b]->Name) == 0)
     {
       ...
       // Size is read from EEPROM and cannot be trusted. An image written by a newer
       // firmware with a larger RFAdata would overflow the allocation.
       if(rfa_data.Size > (int16_t)sizeof(RFAdata)) rfa_data.Size = sizeof(RFAdata);
       if(rfa_data.Size < 0) rfa_data.Size = 0;
...
void RFA_init(int8_t Board, int8_t addr)
{
  ...
  *RFAarray[Board] = RFA_Rev1;
  ...
  SelectedRFAboard = sb;
  // If Rev equals 3 setup the requirements
  if(RFAarray[Board]->Rev == 3)
  {
    // NOTE: the four 18 bit DAC chip select enable bits (RFAcpldCS_*) are deliberately
    // NOT set here. Per the CPLD equations, CS_x asserts on
    //   (BRDSEL AND address match AND latched enable bit)
    // with no gating from the SPI CS line. Leaving an enable latched would let that
    // DAC's select follow the raw address bus. The enables are armed and cleared
    // inside Set_18bitDAC for the duration of a single transaction only.
  }
  ...
  // The CPLD has no reset input (CPLD_CLEA is unconnected), so drive it to a known
  // state explicitly rather than relying on the change detection in RFA_loop.
  SelectBoard(Board);
...
// Rev 3 only. Both poles are always written together because each pole voltage is a
// function of both parameters, so there is no meaningful "set just one" operation.
//
// Reads both parameters from the module structure. Every caller of SetPoleBias and
// SetResolvingDC assigns the structure before calling, and the one call site that
// passes explicit zeros is reached only when the module is disabled, where this
// function zeros both anyway.
// Calculate the RF level detector gain compensation factor from the calibration
// frequency and the current operating frequency. Returns 1.0 when compensation is
// disabled. Extracted from three identical copies in RFA_loop, RFAupdateQUAD and
// RFAacquire.
static float RFAgainComp(int brd)
{
  if(!RFAarray[brd]->EnableComp) return 1.0;
  return 1.0 + (RFAarray[brd]->CalFrequency - RFAarray[brd]->Freq)/1000000.0 * RFAarray[brd]->FreqComp;
}

static void RFAupdatePoles(int brd)
{
   float bias = RFAarray[brd]->PoleBias;
   float rdc  = RFAarray[brd]->ResolvingDC;

   if(!ResolvingDCenable)      rdc = 0;
   if(!RFAarray[brd]->Enabled) bias = rdc = 0;
   SelectBoard(brd);
   Set_18bitDAC(brd, RFAcpldCS_P1, Value2Counts( rdc + bias,&RFAarray[brd]->DACresDCCtrl[0],1.0,RFAdac18BITMAX));
   Set_18bitDAC(brd, RFAcpldCS_P2, Value2Counts(-rdc + bias,&RFAarray[brd]->DACresDCCtrl[1],1.0,RFAdac18BITMAX));
}
...
// This function calculates the voltages to be sent to the DCbias channels used for
// resolving DC. The now flag is an optional value (defaults to false) that will 
...
void SetResolvingDC(int brd, float value, bool now = false)
{
   ...
   if(bd != -1)
   {
     ...
     if(now)
     {
       // Update the DCbias hardware, first select the board
       SelectBoard(bd);
       // Convert the value to DAC counts and output the DAC counts
       AD5668(DCbDarray[bd]->DACspi,DCbDarray[bd]->DCCD[firstch].DCctrl.Chan,Value2Counts(DCbDarray[bd]->DCCD[firstch].VoltageSetpoint,&DCbDarray[bd]->DCCD[firstch].DCctrl),3);
       // Repeat for pole 2
       AD5668(DCbDarray[bd]->DACspi,DCbDarray[bd]->DCCD[firstch+1].DCctrl.Chan,Value2Counts(DCbDarray[bd]->DCCD[firstch+1].VoltageSetpoint,&DCbDarray[bd]->DCCD[firstch+1].DCctrl),3);
     }
     DelayMonitoring();
   }
}
...
void RFA_loop(void)
{
  ...
  for(brd=0;brd<2;brd++)
  {
    ...
    if(AD7998(RFAarray[brd]->ADCadr, ADCvals)==0)
    {
      ...
      {
         val = Counts2Value(ADCvals[RFAadcRFLA],&RFAarray[brd]->ADCchansLR[RFAadcRFLA-RFAadcLR_BASE],gc);
         if(RFAstates[brd]->RFVPpp == -1) RFAstates[brd]->RFVPpp = val;
         else RFAstates[brd]->RFVPpp = Filter * val + (1-Filter) * RFAstates[brd]->RFVPpp;
         val = Counts2Value(ADCvals[RFAadcRFLB],&RFAarray[brd]->ADCchansLR[RFAadcRFLB-RFAadcLR_BASE],gc);
         if(RFAstates[brd]->RFVNpp == -1) RFAstates[brd]->RFVNpp = val;
         else RFAstates[brd]->RFVNpp = Filter * val + (1-Filter) * RFAstates[brd]->RFVNpp;
      }
...
void RFAsetResolvingDC(char *Module, char *value)
{
  ...
  token = value;
  v = token.toFloat();
  if((MIPSconfigData.PowerEnable) && (RFAarray[b]->Enabled) && (ResolvingDCenable)) 
  {
...
void RFAgetResolvingDC(int Module)
{
  ...
  SendACKonly;
  if(SerialMute) return;
  if((MIPSconfigData.PowerEnable) && (RFAarray[b]->Enabled) && (ResolvingDCenable)) serial->println(RFAarray[b]->ResolvingDC,3);
  else serial->println("0.00");
}
...
// Returns the current Ro value in mm
...
// Called by the command processing code, this function performs an update of the resolving DC
// and the RF level. The hardware must be updated before this function returns.
void RFAupdateQUAD(int Module)
{
  ...
  if((b = RFAmodule2board(Module)) == -1) return;
  // This function will calculate and update the resolving DC level and the RF setpoint value.
  // The hardware is not updated by this call.
  QUADupdate(b);
  // Update the resolving DC now
  SetResolvingDC(b, RFAarray[b]->ResolvingDC, true);
  // Update the RF level setpoint now
...
// This function is called from the command processor. Four arguments are required and it is assumed
// they are located in the ring buffer.
//    module, RFA channel 1, or 2
//    m/z, m/z value to set the quad parameters
//    delta, delta to apply to resolving DC
//    dewell, time is mS to wait after setting the parameters before returning
// The function returns the sum of the values in the recorded vection form the previous call.
// The following functions are performed:
//  1.) Read the digitizer vector sum
//  2.) Calculate the quad parameters from the m/z
//  3.) Adjust the resolving DC using the delta
//  4.) Set the RF and DC levels
...
void RFAacquire(void)
{
  ...
  while(true)
  {
    ...
    if(!valueFromCommandLine(&mz,10,100000)) break;
    if(!valueFromCommandLine(&delta,-10,10)) break;
...
void RFAreport(int module)
{
  ...
  SendACKonly;
  if(SerialMute) return;
  // Send all the RF amp parameters, on one comma separated line
...
void quadAutoTune(int brd, bool report)
{
   ...
   if(quadTuning)
   {
      ...
      if(skipFirst)
      {
         // Discard the first reading of a sweep without updating Max or changing the
         // frequency, then take another after a further settling delay. If the module
         // was already tuned and driving a high output, this first reading can still be
         // decaying; letting it set Max would peg FreqMax at the start frequency and
         // nothing measured later would beat it. Belt and braces with quadATstartDelay.
         skipFirst = false;
         Last = Current;
      ...
      switch (state)
      {
        ...
        case QUAD_AT_DWN_SWR:
          ...
          qdly = (TuneStep <= 500) ? quadATfineDelay : quadATstepDelay;
          break;
       case QUAD_AT_UP:
       case QUAD_AT_UP_SWR:
          if(Current > Max)
          ...
          if((NumDown >= QUAD_AT_MAX_DWN) || (RFAarray[brd]->Freq > quadATmaxF))
          {
            ...
            if(TuneStep <= 500)
            {
              ...
              {
                RFAarray[brd]->Drive = quadAThighP;
                state = QUAD_AT_UP;
              }
              if(state == QUAD_AT_UP)
              {
              ...
              quadTuning = false;
              // Sanity checks. Deliberately NOT gated on 'report': a tune that converged
              // on something other than the tank resonance is a failure, and a failure
              // indication must not be optional. Only SerialMute suppresses it.
              bool suspect = false;
              if((Freq <= quadATminF) || (Freq >= quadATmaxF)) suspect = true;
              if(((Freq - StartFreq) < quadATmaxStep) && ((StartFreq - Freq) < quadATmaxStep)) suspect = true;
              if(suspect)
              {
                DisplayMessage("Tune suspect!", 2000);
                if(!SerialMute)
                {
                  serial->print("WARNING: auto tune result is suspect, frequency = ");
                  serial->print(Freq);
                  serial->print(", peak metric = ");
                  serial->println(Max);
                  serial->println("         The tune stopped at a search range limit or within one coarse step");
                  serial->println("         of the start frequency, so the peak was probably never seen.");
                  serial->println("         If the module was already tuned and driving a high output, increase");
                  serial->println("         SRFATSTARTDLY so the RF level decays before the sweep begins.");
                  serial->println("         If the tank Q is high, narrow SRFATMINF/SRFATMAXF or reduce SRFATSTEP.");
                }
              }
...
void setRFArev3(int module)
{
  ...
  RFAarray[b]->ADCresDCCtrl[DCmonHVP].m = 100;
  RFAarray[b]->ADCresDCCtrl[DCmonHVP].b = 0;
  // Negative HV readback
  RFAarray[b]->ADCresDCCtrl[DCmonHVN].m = -100;
  RFAarray[b]->ADCresDCCtrl[DCmonHVN].b = 65535;
  for(int i=0;i<4;i++) RFAarray[b]->ADCresDCCtrl[i].Chan = i;
  SendACK;
}

// The calRFApoleSupplies function calibrates the pole supplies for a specified module by measuring voltages, 
// adjusting digital-to-analog converters (DACs), and calculating calibration parameters based on user input 
// and readback values from analog-to-digital converters (ADCs). It ensures proper board selection and handles 
// high voltage power supply operations while providing serial output for monitoring the calibration process.
void calRFApoleSupplies(int module)
{
  int b;
  #define   CNTS_P1 131000
  #define   CNTS_P2 200000

  if((b = RFAmodule2board(module)) == -1) return;
  if(RFAarray[b]->Rev != 3) BADARG;
  // Capture the supply state so it can be restored on exit
  bool hvWasOn = (digitalRead(PWR_ON) == LOW);
  // Turn on the HV power supply
  digitalWrite(PWR_ON,LOW);
  for(int i=0;i<2;i++)
  {
    SelectBoard(b);
    serial->print("Calibrating Pole "); serial->println(i+1);
    // Set zero point and ask for actual value
    if(i==0) Set_18bitDAC(b, RFAcpldCS_P1, CNTS_P1);
    if(i==1) Set_18bitDAC(b, RFAcpldCS_P2, CNTS_P1);
    ...
    if(i==1) Set_18bitDAC(b, RFAcpldCS_P2, CNTS_P2);
    float V2 = UserInputFloat("Enter measured pole voltage : ", NULL);
    SelectBoard(RFAarray[b]->rbBoard);
    int Vadc2 = AD7994(RFAarray[b]->rbADDR, RFAarray[b]->ADCresDCCtrl[i].Chan, 20);
    // Generate calibration parameters
    // count = value * m + b
    RFAarray[b]->ADCresDCCtrl[i].m = (Vadc2 - Vadc1) / (V2 - V1);
  ...
  RFAarray[b]->ADCresDCCtrl[DCmonHVP].b = VP2adc - VP2 * RFAarray[b]->ADCresDCCtrl[DCmonHVP].m;
  ...
  serial->print(" b = "); serial->println(RFAarray[b]->ADCresDCCtrl[DCmonHVN].b);

  // Restore the supply to its entry state. The procedure turns it off to read the
  // power-off points and previously left it off.
  digitalWrite(PWR_ON, hvWasOn ? LOW : HIGH);
  // Force the loop to resync the poles; the DACs were left at the calibration values.
  RFAupdate = true;
  SendACK;
}