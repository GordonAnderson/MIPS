...
#define  Filter 0.5         // Weak filter coefficient
...
void SetChannelOffset(int brd, float fval)
{
  ...
  if(SelectedDCBoard == brd) dcbd.ChannelOffset = fval;  // Sets the UI buffer if this module is selected
  if(digitalRead(PWR_ON) != 0) return;
  AcquireTWI();
  int b = SelectedBoard();
  SelectBoard(brd);
  ...
  for(b=0;b<4;b++)
  {
    ...
    else for(i=0;i<DCbDarray[b]->NumChannels;i++)
    {
      if(!ValueChange)
      {
         if((DCbDarray[0] != NULL) && (DCbDarray[0]->OffsetReadback) && (i == 0))   // Added i==0 April 6, 2019.
         {
           if((DCbDarray[b]->DACadr & 0xFE) == 0x10)
           {
              ...
              if((DCbDarray[0]->UseOneOffset) && (*(int *)&DCbDarray[b]->UseOneOffset != 2))
              {
                ...
                //if(b == lastModule)  { offsetV = Counts2Value(ov =AD5593readADC(DCbDarray[b]->DACadr,DCbDarray[b]->DCoffset.DCmon.Chan),&DCbDarray[b]->DCoffset.DCmon); }
                SelectBoard(lastModule);
                offsetV = Counts2Value(ov =AD5593readADC(DCbDarray[lastModule]->DACadr,DCbDarray[lastModule]->DCoffset.DCmon.Chan),&DCbDarray[lastModule]->DCoffset.DCmon);
                SelectBoard(b);
         ...
         // Updated 8/4/2024 to test offset separate from the channels
         if(i == 0)
         {
            flt = 0.5;
            if(abs(filteredOffset[b] - offsetV) < 2) flt = 0.1;
...
// This function enabled toggling between two different DCbias voltage profiles.
...
void profileLevelISR(void)
{
  delayMicroseconds(10);
  if(digitalRead(LT) == LOW) ApplyDCbiasProfile(Profile1);
  else ApplyDCbiasProfile(Profile2);
}

// Profile toggle command line argument options:
// profile1,profile2,dwell time (mS)  // Toggle profile change
// profile1,profile2,R                // Trigger profile change on R input
// profile1,profile2,L                // Set profile 1 when level sensor pulse is less than 10 uS wide
...
void SetDCbiasProfileToggle(void)
{
   ...
   while(1)
   {
     ...
     if(sToken == "L")
     {
       // Here if the user selected the L input to control the toggle
       if(ProfileTrig == NULL) ProfileTrig = new DIhandler;
       pinMode(LT,INPUT);
       attachInterrupt(LT,profileLevelISR,RISING);
       SendACK;
       return;
     }
...
void SetDCbiasCHMK(char *brd, char *mask)
{
  ...
  if(SelectedDCBoard == board) dcbd.OffsetChanMsk = DCbDarray[board]->OffsetChanMsk;  // Sets the UI buffer if this module is selected
