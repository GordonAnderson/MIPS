...
// Hardware
//
//  This file contains low level hardware IO drivers. This includes analog and digital IO devices.
...
extern int  NumberOfFilamentChannels;

// Forward declarations for cross-file ISR/callback functions
...
// Clears the digital output shift registers.
...
// This function reads the 8 digital input lines and packs the data into one unsigned
...
//        the values are 0 to 65535 regardless of ADC resolution
//
...
int AD7994_b(int8_t adr, int8_t chan)
{
  ...
  val &= 0xFFF;
  val <<= 4;
 // if(Wire.endTransmission() !=0). // June 24, 2025
 // {
 //   TWIerror();
 //   TWIfails++;
 //   ReleaseTWI();
...
// AD5593 IO routines. This is a analog and digital IO chip with
// a TWI interface. The following are low level read and write functions,
// the modules using this device are responsible for initializing the chip.
...
// AD5593 IO routines for Wire1 interface. This is a analog and digital IO chip with
// a TWI interface. The following are low level read and write functions,
// the modules using this device are responsible for initializing the chip.
...
// Cmd = 3 to update when written regardless of LDAC
...
void SetDelayTrigModule(char *module)
{
  ...
  else if(Module == "ADC") DelayedTrigFunction = ADCdelayedTriggerCallback;
...
bool LevelDetWindowDetected = false;
...
void LevelDetWindowISR(void)
{
  LevelDetWindowDetected = true;
}
...
void LevelDetChangeReport(void)
{
  ...
  {
    LevelDetWindowDetected = false;
    // Read the window state and report
    Wire1.beginTransmission(LevelDetAdd);
    Wire1.write(TWI_LEVDET_READ_WSTATE);
    Wire1.endTransmission();
    Wire1.requestFrom((uint8_t)LevelDetAdd, (uint8_t)1);
...
// The address is in hex of OFF to disable
void SetLevelDetChangeReport(char *TWIadd)
{
  if(strcmp(TWIadd,"OFF")==0)
  {
    detachInterrupt(digitalPinToInterrupt(LEVCHANGE));
    SendACK;
    return;
  }
...
void SetLevelDetWindowReport(char *TWIadd)
{
  if(strcmp(TWIadd,"OFF")==0)
  {
    detachInterrupt(digitalPinToInterrupt(LEVCHANGE));
  ...
  Wire1.begin();
  Wire1.setClock(Wire1DefaultSpeed);
  attachInterrupt(digitalPinToInterrupt(LEVCHANGE), LevelDetWindowISR, RISING);
  SendACK;
}
