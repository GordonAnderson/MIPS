...
// DIO.cpp
//
// This file supports the Digital input and output functions on the MIPS system. The MIPS controller
// has 16 digital output channels and 8 digital inputs.
//
// Digital outputs are named A though P.
// Digital inputs are named Q through X.
//
...
uint32_t  DIOholdOff = 0;
uint32_t  DIOreportAfter;
...
void UpdateDigitialOutputArray(void)
{
  ...
  // Update the Digital output array using the image registers
  for (i = 0; i < 8; i++)
  {
...
void DIO_loop(void)
{
  ...
  // Read input bits and set digital input array for the display
  ...
  // If the digital input dialog is displayed then update the data
...
// P are digital outputs and Q through X are digital inputs.
void GDIO_Serial(char *CH)
{
  ...
  if ((CH[0] >= 'A') && (CH[0] <= 'X'))
  {
    ...
    if (CH[0] >= 'Q')
    {
      // Here if this is a digital input channel. Read the input and respond
      DIbits = DigitalIn();
    ...
    // Here if this is a digital output. In this case read the image register
...
void ProcessTriggerOut(void)
{
  if(TriggerOutQueued)
  {
    ...
    if(TriggerOutmicroSec == 0) digitalWrite(TRGOUT, HIGH);
    else if(TriggerOutmicroSec == -1) digitalWrite(TRGOUT, LOW);
    else TriggerOut(TriggerOutmicroSec);
  }
}
...
//    if((dioops[i].Report) || (dioops[i].Mirror))
    {
      ...
      if(j != dioops[i].LastLevel)
      {
        ...
        if(!dioops[i].Mirror)
        {
          DIOreportAfter = DIOholdOff + millis();
