...
// The following commands are processed when the power to the MIPS system is off
const Commands  OffCmdArray[] = 	{
// General commands
  {"GVER",  CMDstr, 0, (char *)Version},                 // Report version
  {"ISPWR",  CMDstr, 0, (char *)OffMessage},             // Report power status
  {"GNAME", CMDstr, 0, (char *)MIPSconfigData.Name},	   // Report MIPS system name
  {"RESET", CMDfunction, 0, (char *)Software_Reset},     // Reset the Due  
  {"POWER", CMDfunction, 0, (char *)PowerControl},       // Cycle MIPS system power, if supported
// End of table marker
  {0},
};
...
// 
// MIPS host interface commands. These commands are grouped into
// catagories. This list includes commands for the common MIPS functions
// and modules.
// 
// General commands
  ...
  {"THREAD", CMDfunctionStr, 1, (char *)ThreadDetails},     // List details on the named thread
  ...
  {"RUNNOW", CMDfunctionStr, 1, (char *)RunThreadNow},         // Run the thread defined by name, now
  {"THRDRESTART", CMDfunction, 0, (char *)RestartAllThreads},  // Restart all threads
  ...
  {"CREATE", CMDfunctionStr, 1, (char *)CreateFile},           // Create an empty file, fails if file is present
  {"ADD", CMDfunctionLine, 0, (char *)AddFile},                // Add a string to the filename passed
  {"MORE", CMDfunctionStr, 1, (char *)MoreFile},               // List file to serial port
  {"READSD", CMDfunctionStr, 2, (char *)ReadLine},             // Read line number from file, 0 is first line
...
// TWI commands
  ...
  {"GTWI", CMDfunctionStr, 1, (char *)gtwi},                   // Reads value from TWI device
  {"STWI", CMDfunctionStr, 2, (char *)stwi},                   // Writes a value to a TWI device
// Log commands
...
// Level detection module change enable
  {"ENALDET", CMDfunctionStr, 1, (char *)SetLevelDetChangeReport},  // Enables the level detection module to report a detected change to the host.
                                                                    // This function requires the module address in hex. OFF to disable
  {"ENAWLDET", CMDfunctionStr, 1, (char *)SetLevelDetWindowReport}, // Enables the level detection module to report window state to the host.
                                                                    // This function requires the module address in hex. OFF to disable
...
// ADC functions
  ...
  // ADC record vector to ram buffer commands, implemented Aug 2, 2025
  {"ADCRBINIT", CMDfunction, 0, (char *)ADCrbInit},            // ADC record vector initilization
  {"ADCRBTRIG", CMDfunction, 0, (char *)ADCrbTrig},            // ADC record vector trigger
  {"ADCRBREAD", CMDfunction, 2, (char *)ADCrbRead},            // ADC record vector read, start,num
  {"ADCRBSUM", CMDfunction, 0, (char *)ADCreadSum},            // Reports the sum of all values in the vector recorded
  {"ADCRBMAX", CMDfunction, 0, (char *)ADCreadMax},            // Reports the maximum value in the vector recorded
  {"ADCRBVECS", CMDfunction, 0, (char *)ADCvectorsRead},       // Reports the number of vectors recorded
...
// RF driver module commands
  ...
  {"RFPWLC", CMDfunctionStr, 2, (char *)genPWLcalTable}, // Generate piecewise linear calibration table for selected channel and phase
                                                         // Phase is RF+ or RF-
  {"SRFTFR", CMDbool, 1, (char *)&TuneRFfullRange},      // If TRUE the first step of auto turn will use the full range
...
// DIO module commands
  ...
  {"SHOLDOFF", CMDint, 1, (char *)&DIOholdOff},             // Set DIO change reporting holdoff in mS
  {"GHOLDOFF", CMDint, 0, (char *)&DIOholdOff},             // Report DIO change reporting holdoff in mS
...
// Table commands, tables enable pulse sequence generation
  ...
  {"STBLTSKS",CMDbool, 1, (char *)&TblTasks},              // Set to TRUE to enable tasks in table mode based on available time, FALSE is default
  ...
  {"TBLCHK",CMDfunction, 0, (char *)TableCheck},           // The function tests a table for timing violations and prints the results
...
void TWIaddSet(char *module)
{
   ...
   if(sToken == "RFD")
   {
     ...
     // -Look for a AD7988 at all possible addresses, 20,21,22,23,24
     // -Look for a ADS7828 at all possible addresses, 48,49,4A,4B
...
void SetTime(void)
{
  ...
  while(true)
  {
     ...
     s = arg.toInt();
     if(s >= 60) break;
     rtc.write(day, month, year, h, m, s, isPM, is12H);
     SendACK;
     return;
  }
  BADARG;
...
void SetDate(void)
{
  ...
  while(true)
  {
     ...
     y = arg.toInt();
     if(y<1970) break;
     rtc.write(d, m, y, hour, minute, second, isPM, is12H);
     SendACK;
     return;
  }
  BADARG;
...
// designed USB power will always power the processor regardless of this setting.
...
{
  wire->begin();
  if(wire == &Wire) wire->setClock(WireDefaultSpeed);
  if(wire == &Wire1) wire->setClock(Wire1DefaultSpeed);
...
void RestartAllThreads(void)
{
  int    i = 0;
  Thread *t;

  SendACKonly;
  serial->println("Restaring all threads.");
  while (1)
  {
    t = control.get(i++);
    if (t == NULL) break;
    serial->println(t->getName());
    t->setNextRunTime(millis() + 25 * i);
  }
}
...
// This function allows enable or disabling a thread.
// The thread is identified by name.
...
void ThreadDetails(char *name)
{
  ...
  if (t == NULL) BADARG;
  SendACKonly;
  // Report this treads details:
  serial->print("Thread: "); serial->println(t->getName());
  serial->print("ID: "); serial->println(t->getID());
  serial->print("now: "); serial->println(millis());
  serial->print("interval: "); serial->println(t->getInterval());
  serial->print("run time: "); serial->println(t->runTimeMs());
  serial->print("enabled: "); serial->println(t->enabled);
}
...
// This function returns a pointer to a c style string that contains a comma separated
