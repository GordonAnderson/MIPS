...
//    - Direction
//    - Arb waveform
//  - Added TWI acquire and release around all functions
...
//      to ARB module.
//
// This module driver has been updated to support 4 total channels. This required the 
// following updates:
//  - Use a array of pointers to ARB structures and allocate at init time.
//  - Allocate compressor table space at run time
//  - Redesign module selection for 4 boards. Module has only one bit for board
//    select so EEPROM address has to change for second pair. The module has only
//    one device address, the ARB DUE. This is a TWI address and is different
//    for second pair. As ARB command allows you to define this address.
//  - At board init time if channels 1 and 2 are already present then the next two
//    ARB board inits are channels 3 and 4 at secondary TWI addresses. 
...
// ARBs. 
//    ARBsync (9), connects to A0 on Arduino based ARB, A6 on rev 4.2, left off on 4.1
...
// Forward declarations for ISR functions and callbacks used in static initializers
void ARB_1_SWEEP_ISR(void); void ARB_2_SWEEP_ISR(void); void ARB_3_SWEEP_ISR(void);
void ARB_4_SWEEP_ISR(void); void ARB_5_SWEEP_ISR(void); void ARB_6_SWEEP_ISR(void);
void ARB_1_DIR_ISR(void);   void ARB_2_DIR_ISR(void);   void ARB_3_DIR_ISR(void);
void ARB_4_DIR_ISR(void);   void ARB_5_DIR_ISR(void);   void ARB_6_DIR_ISR(void);
...
{
  if(strcmp(mode,"TWAVE") == 0) SetByte(board, TWI_ARB_SET_MODE, 0);
  else SetByte(board, TWI_ARB_SET_MODE, 1);
}
...
void SetChans(void)
{
  SetFloat(SelectedARBboard, TWI_ARB_SET_SET_BUFFER, ChanLevelAll);
}
...
void SetChannelRangeMessage(int board, int channel, int strtI, int stpI, float val)
{
  ...
  Wire.write(TWI_ARB_SET_SET_CHN_RNG);
...
void SetBufferLength(int board, int BufferLength)
{
  ...
  AcquireTWI();
  Wire.beginTransmission(ARBarray[board]->ARBadr);
  Wire.write(TWI_ARB_SET_BUFFER_LEN);
  Wire.write(b[0]);
  Wire.write(b[1]);
  {
    AtomicBlock< Atomic_RestoreState > a_Block;
...
void SetNumBuffers(int board, int NumBuffers)
{
  ...
  AcquireTWI();
  Wire.beginTransmission(ARBarray[board]->ARBadr);
  Wire.write(TWI_ARB_SET_NUM_BUFFER);
  Wire.write(b[0]);
  Wire.write(b[1]);
  {
    AtomicBlock< Atomic_RestoreState > a_Block;
...
// Accepts a waveform type descriptive string and returns the type
...
void SetAmplitude(int board, float Voltage)
{
  ...
  SetFloat(board, TWI_ARB_SET_RANGE, Voltage);
}

// This command support the dual output amplifier board option. Each board
// can have a separate offset. Add = 0 or 1 for board A or B.
...
void SetWaveform(int board, WaveFormTypes wft)
{
  ...
  SelectBoard(board);
  Wire.beginTransmission(ARBarray[board]->ARBadr);
  Wire.write(TWI_ARB_SET_WAVEFORM);
  switch (wft)
  {
    case ARB_SIN:
...
//              serial->println(ARBarray[i]->Frequency);
      ...
      {
        SetBool(b, TWI_ARB_SET_DIR, !ARBarray[b]->Direction);
      ...
      {
        SetBool(b, TWI_ARB_SET_ENABLE, ARBarray[b]->Enable);
...
void SetARBUseCommonClock(char *module, char *flag)
{
  ...
  if((smode == String("TRUE")) || (smode == String("FALSE")))
  {
     ...
     if(smode == String("TRUE")) 
     {
        ...
        SetBool(b, TWI_ARB_SET_EXT_CLOCK, true);
     }
     else  
     {
        ARBarray[b]->UseCommonClock = false;
        SetBool(b, TWI_ARB_SET_EXT_CLOCK, false);
     }
     SendACK;
     return;
  }
...
void SetWFdisable(int module)
{
  ...
  SetBool(b,TWI_ARB_SET_ENABLE,false);
}
...
void SetWFenable(int module)
{
  ...
  SetBool(b,TWI_ARB_SET_ENABLE,true);
}
...
void SetWFoffsetV(char *module, char *srange)
{
  ...
  if((range >= -50) && (range <= 50))
  {
    ...
    SetFloat(b, TWI_ARB_SET_OFFSETV, range);
    SendACK;
    return;
  }
  SetErrorCode(ERR_BADARG);
...
void SetWFaux(char *module, char *srange)
{
  ...
  if((range >= -50) && (range <= 50))
  {
    ...
    SetFloat(b, TWI_ARB_SET_AUX, range);
    SendACK;
    return;
  }
  SetErrorCode(ERR_BADARG);
...
// The following functions support the dual output board option available on rev 3.0
...
// This functions needs to be updated to first put all ARBs in remote sync mode and then
// toggle the sync line. The ARB sync mode must be restored before exit
...
void UpdateAux(int8_t brd, float val, bool FlushQueued)
{
  ...
  if(FlushQueued)
  {
    // If here assume the TWI interface is available so send any queued data
    int b=SelectedBoard();
    for(int i=0; i<4; i++) 
    {
      if(Queued[i])
      {
         SetFloat(i, TWI_ARB_UPDATE_AUX, vals[i]);     
         ARBupdatesSent |= 1 << i;
      }
    }
    SelectBoard(b); 
  ...
  if(AcquireTWI())
  {
    // Here if TWI interface is available
    int b=SelectedBoard();
    SetFloat(brd, TWI_ARB_UPDATE_AUX, val);
    SelectBoard(b);           
...
void UpdateOffsetA(int8_t brd, float val, bool FlushQueued)
{
  ...
  if(FlushQueued)
  {
    ...
    for(int i=0; i<2; i++) 
    {
      if(Queued[i])
      {
         SetBoardBias(i, 0, vals[i], TWI_ARB_UPDATE_BRD_BIAS); 
         ARBupdatesSent = 1 << i;        
      }
    }
    SelectBoard(b); 
  ...
  if(AcquireTWI())
  {
    // Here if TWI interface is available
    int b=SelectedBoard();
    SetBoardBias(brd, 0, val, TWI_ARB_UPDATE_BRD_BIAS);    
    SelectBoard(b);           
    ARBupdatesSent |= 1 << brd;
  }
  else
...
void UpdateOffsetB(int8_t brd, float val, bool FlushQueued)
{
  ...
  if(FlushQueued)
  {
    ...
    for(int i=0; i<2; i++) 
    {
      if(Queued[i])
      {
        SetBoardBias(i, 1, vals[i], TWI_ARB_UPDATE_BRD_BIAS); 
        ARBupdatesSent = 1 << i;
      }
    }
    SelectBoard(b); 
  ...
  if(AcquireTWI())
  {
    // Here if TWI interface is available
    int b=SelectedBoard();
    SetBoardBias(brd, 1, val, TWI_ARB_UPDATE_BRD_BIAS);    
    SelectBoard(b);           
    ARBupdatesSent |= 1 << brd;
  }
  else
...
void ProcessARB(void)
{
  ...
  // If the TWI is not available then queue this function to
  ...
  for(int i=0; i<MAXARBMODULES; i++)
  {
      if(ARBupdatesSent & (1 << i))
      {
         ...
         Wire.write(TWI_ARB_LOAD_UPDATES);
         {
            AtomicBlock< Atomic_RestoreState > a_Block;
            Wire.endTransmission();
         } 
...
void GetARBversion(int module)
{
  int b;
   ...
   float fVer;
   if(ReadFloat(b,TWI_ARB_READ_VERSION,&fVer))
   {
...
void ARBstopSweep(int module)
{
  ...
  SetByte(b, TWI_ARB_SWPGO, SS_STOP);
  SendACK;  
}
...
void GetARBsweepStatus(int module)
{
  ...
  int i;
  if(!Read8bitUnsigned(b,TWI_ARB_READ_SWEEP_STATUS, &i)) return;
  switch (i)
  {
    case SS_IDLE:
      serial->println("IDLE"); 
