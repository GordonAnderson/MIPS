...
#define  Filter 0.05         // Strong filter coefficient

//MIPS Threads
Thread RFdriverThread  = Thread();

RFdriverData  *rfddarray[MAXRF] = {NULL,NULL,NULL,NULL};
bool RFdriverModel2[MAXRF] = {false,false,false,false};  // Set to true in init if a model 2 RFdriver module is detected
...
// and limit testing.
// Variables used for model 1 RFdriver boards
float RFpVpps[2][2];
float RFnVpps[2][2];
float Powers[2][2];
// Bariables used for model 2 RFdriver boards
...
void RFat(void)
{
  ...
  int brd = BoardFromSelectedChannel(SelectedRFChan);
  if(brd == -1) return;
  if(RFdriverModel2[brd])
  {
...
// SC the selected channel range is 0 through MAXRF-1
// Returns -1 if error condition is detected
int8_t BoardFromSelectedChannel(int8_t SC)
{
  for(int brd=0;brd<MAXRF;brd++)
  {
    if(rfddarray[brd] != NULL)
    {
      if(SC < 2) return(brd);
      SC -= 2;
    }
...
void SelectChannel(void)
{
  ...
  SelectBoard(SelectedRFBoard);
  if(RFdriverModel2[SelectedRFBoard])
  {
    // Turn off the gate menu options
    RFdriverDialogEntriesPage2[3].Type = D_OFF;
    RFdriverDialogEntriesPage2[4].Type = D_OFF;
    // Set the power limit
    RFdriverData2 *rfd = (RFdriverData2 *)RFDD;
    RFdriverDialogEntriesPage2[1].Max = rfd->PowerLimit;
  }
  else
  {
    // Turn on the gate menu options
    RFdriverDialogEntriesPage2[3].Type = D_LIST;
    RFdriverDialogEntriesPage2[4].Type = D_LIST;
...
void RFdriver_init(int8_t Board, int8_t addr)
{
  ...
  // Determine if this is a model 2 module connected via TWI
  if(TWItest(RFdrvCMDadd(addr)))
  {
    if(rfddarray[Board] != NULL) Board += 2;
    RFdriverModel2[Board] = true;
    ...
    strcpy(rfddarray[Board]->Name,"RFdriver");
    // Turn off the gate menu options
    RFdriverDialogEntriesPage2[3].Type = D_OFF;
    RFdriverDialogEntriesPage2[4].Type = D_OFF;
  }
  ...
  {
    // Model 1 module, this is the orginal configuration and only 2 modules are supported.
    if(rfddarray[Board] != NULL) return;  // Exit if in use
    RFdriverModel2[Board] = false;
    rfddarray[Board] = new RFdriverData;
    // Init the structure
    if(Board == 0) *rfddarray[Board] = RFDD_A_Rev_1;
    if(Board == 1) *rfddarray[Board] = RFDD_B_Rev_1;
    // Turn off the gate menu options
    RFdriverDialogEntriesPage2[3].Type = D_LIST;
    RFdriverDialogEntriesPage2[4].Type = D_LIST;
  }
  ...
  //serial->println(RFDDarray[Board]->EEPROMadr);
  if (NormalStartup)
  {
    RestoreRFdriverSettings(true);
  }
  if(RFdriverModel2[Board] == false)
  {
...
//  TUNE_SCAN_DOWN
//  TUNE_SCAN_UP
// Only valid for model 1 drivers
void RFdriver_tune(void)
{
   ...
   switch (TuneState)
   {
     ...
     case TUNE_SCAN_UP:
        ...
        if((TuneRFfullRange) && (TuneStep == MAXSTEP)) NumDown = 0;
        RFDDarray[TuneRFBoard]->RFCD[TuneRFChan & 1].Freq += TuneStep;
...
void RFdriver_loop_model1(void)
{
  ...
  for (i = 0; i < NumberOfRFChannels; i++)
  {
    ...
    if(RFdriverModel2[SelectedRFBoard]) continue;
    SelectBoard(SelectedRFBoard);
...
void RFdriver_loop_model2(void)
{
  float fval;
  int   ival;
  ...
  { 
    ...
    if(RFdriverModel2[b] == false) continue;
...
void RFdriver_loop(void)
{
  ...
  // Call the processing loop based on models in system
  for(int brd=0;brd<MAXRF;brd++)
  {
    if((rfddarray[brd] != NULL) && (RFdriverModel2[brd] == false))
    {
      RFdriver_loop_model1();
      break;
    }
  }
  for(int brd=0;brd<MAXRF;brd++)
  {
    if((rfddarray[brd] != NULL) && (RFdriverModel2[brd]))
    {
...
// Reports the number of RF output channels available.
void RFnumber(void)
{
  SendACKonly;
...
void RFdrive(char *Chan, char *Val)
{
  ...
  if ((Drive < 0) || (Drive > RFDDarray[i]->RFCD[(channel - 1) & 1].MaxDrive)) BADARG;
...
void RFautoTune(int channel)
{
  ...
  if(RFdriverModel2[i])
  {
    ...
    TWIsetByte(RFdrvCMDadd(RFDDarray[i]->EEPROMadr), i, TWI_RF_SET_CHAN, (TuningChannel & 1) +1);
    TWIcmd(RFdrvCMDadd(RFDDarray[i]->EEPROMadr), i, TWI_RF_SET_ATUNE);
    DisplayMessage("Tune in process");
  }
  else
  {
...
void RFautoRetune(int channel)
{
  ...
  if(RFdriverModel2[i])
  {
    ...
    TuningChannel = channel-1;
    TWIsetByte(RFdrvCMDadd(RFDDarray[i]->EEPROMadr), i, TWI_RF_SET_CHAN, (TuningChannel & 1) +1);
    TWIcmd(RFdrvCMDadd(RFDDarray[i]->EEPROMadr), i, TWI_RF_SET_RTUNE);
    DisplayMessage("Retune in process");
  }
  else
  {
...
void RFvoltageReportP(int channel)
{
  ...
  i = BoardFromSelectedChannel(channel - 1);
  if(SerialMute) return;
  if(RFdriverModel2[i]) serial->println(RFrb[i][(channel - 1) & 1]->RFP);
  else serial->println(RFpVpps[i][(channel - 1) & 1]);
}
...
void RFvoltageReportN(int channel)
{
  ...
  i = BoardFromSelectedChannel(channel - 1);
  if(SerialMute) return;
  if(RFdriverModel2[i]) serial->println(RFrb[i][(channel - 1) & 1]->RFN);
  else serial->println(RFnVpps[i][(channel - 1) & 1]);
}
...
void RFheadPower(int channel)
{
  ...
  i = BoardFromSelectedChannel(channel - 1);
  if(SerialMute) return;
  if(RFdriverModel2[i]) serial->println(RFrb[i][(channel - 1) & 1]->PWR);
  else serial->println(Powers[i][(channel - 1) & 1]);
}
...
void GetRFpwrLimit(int channel)
{
  ...
  if(RFdriverModel2[i])
  {
    RFdriverData2 *rfd = (RFdriverData2 *)RFDDarray[i];
    serial->println(rfd->PowerLimit);
  }
  else serial->println(RFDDarray[i]->PowerLimit);
}
...
void SetRFpwrLimit(int channel, int Power)
{
  ...
  {
    RFdriverData2 *rfd = (RFdriverData2 *)RFDDarray[i];
    rfd->PowerLimit = Power;
    if(i == SelectedRFBoard)
    {
      RFdriverData2 *rfd = (RFdriverData2 *)RFDD;
      rfd->PowerLimit = Power;
    }
...
void RFgateDisable(char *cmd)
{
  ...
  if(sToken == "TRUE")
  {
    ...
    {
      brd = BoardFromSelectedChannel(i);
      if(rfddarray[brd] == NULL) continue;
      if(RFdriverModel2[brd]) continue;
      SelectBoard(brd);
...
void setupADCRFcontrol(char *chan, char *gain)
{
  ...
  if(BoardFromSelectedChannel(adc)==-1) BADARG;
  if(RFdriverModel2[BoardFromSelectedChannel(adc)]) BADARG;
...
void getRFgatedDrv(int Chan)
{
  ...
  if(RFdriverModel2[BoardFromSelectedChannel(Chan - 1)]) BADARG;
  SendACKonly;
  if(SerialMute) return;
