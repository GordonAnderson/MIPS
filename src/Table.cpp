...
//
// To do, March 2025
//  1.) Remove table.cpp and make table2.cpp the default, also remove
//      the compile switch.
//  2.) Update the code to allow a time event in a table loop that is 
//      longer than the loop time. In this case [A:100,0:1:1.5,75:2:35,50:]
//      the event at time point 75 would happen only one time on the second
//      pass through the table.
//  3.) Allow time points to be out of order, rewrite the advance time point
//      function, it needs to use delta time for sweeping
//
...
//
// Conditional logic
//   If a table name is defined as P then the table loop conditional testing is enabled. In this mode
//   a channel can be preceded by a logic test of the loop count. If the test is true then the next channel
//   command will be executed. The conditional test only applies to the channl following the conditional test.
//   Digital outputs do not function in the P tsblr conditional logic mode. The following additional channel
//   commands have been added:
//    if channel is 40 = Trigger ADC
//    if channel is 41 = Trigger output high
//    if channel is 42 = Trigger output low
//   Below are a few examples:
//     =:7:2:-12    This will set output channel 2 to -12 is the loop count is 7
//     >:3:1:11     If the loop counter is above 3 then channel 1 is set to 11
//   This only applies to a table named P and it must be configured for multiple loops.
//
...
bool TblTasks = false;                // If true enables processing tasks when there is available time during table execution
int  ExtFreq = 0;                     // Defines enternal clock frequency used by the table, needed for the TblTasks mode.

// The following variables are used to support the loop time incrementing or decrementing. This capability
// was added October 25, 2017, and redesigned December 6, 2020
...
bool TrigEvyCycle = false;            // If this flag is set and we are in external trigger mode and also re-trigger
                                      // is enabled then the table generation stops after each sub table
...
void ParseTableCommand(void)
{
    ...
    if((TableMode == TBL) && (strcmp(TableStatus,"READY")!=0))
    {
    ...
    while(1)
    {
        ...
        // At this point the table is initialized and i has the first time
...
int ParseEntry(int Count, char *TK)
{
    ...
    // Define Entry pointer and initialize
    TEH = (TableEntryHeader *) &(VoltageTable[CT][ptr]); ptr += sizeof(TableEntryHeader);
    TEH->Count = Count;
    TEH->NumChans = 0;
...
void ProcessTasks(void)
{
  ...
  // If the timer is running test the available time
  {
...
void ProcessTables(void)
{
    ...
    if(!DisableDisplay) DisplayMessage("Table Mode Enabled");
    StopCommanded = false;
    StartRampClock();
    ...
    while(1)
    {
        ...
        while(1)
        {
            ...
            // Process tasks if time is available
            ProcessTasks();
            // Restart table if requested by user
            if(StopCommanded)
            {
...
//            if(StopRequest == true) break;     // removed 5/5/18
    ...
    SetImageRegs();
    if(!DisableDisplay) DismissMessage();
    CT = InitialTableNum;
    DCbiasUpdate = true;
}
...
// supports the maximum possible performance. This codes does not support some of the advanced
...
SetupNextEntryAgain2:
    ...
    while(1)
    {
        ...
        {
            // Table named P loop counter conditionals code, if the
            // condition is met then next channel command is executed
            // example =:12:1:45, if loop count is 12 then channel 1 to 45 volts.
            // Updated 8/20/25 to support logical and of two series conpars, for example
            // >:60:<:80:1:34, in this case is the loop count is greater than 80 and less than 80
            // then channel 1 is set to 34 volts
            if(Theader->TableName == 'P')
            {
              if(Tentry[i].Chan == '=')
              {
                ...
                //else {i++; continue;}
                else {if((++i < TEheader->NumChans) && ((Tentry[i].Chan == '<') || (Tentry[i].Chan == '>'))) i++; continue;}
              }
              if(Tentry[i].Chan == '>')
              {
                if(NS.Count[NS.Ptr-1] > Tentry[i].Value) continue;
            ...
            else if((tableBasedRamping) && (((Tentry[i].Chan & 0x3F) >= 0) && ((Tentry[i].Chan & 0x3F) <= 31)))
            {
              AtomicBlock< Atomic_RestoreState > a_Block;
              // See if the SPI address is correct, if not update
            ...
            {
              if(Tentry[i].Chan == 40) ADCrbTrigger();                 // Trigger ADC digitizer
              if(Tentry[i].Chan == 41) pioTrig->PIO_CODR = pinTrig;    // Set Tgigger output low;
              if(Tentry[i].Chan == 42) pioTrig->PIO_SODR = pinTrig;    // Set Trigger output high;
            }
