...
//   - Use a char string holding a list of comma separated list names.
//   - *fmt points to the list of names
//   - *Value point to selected name
//   - Number of commas + 1 defines the number of entries in list
...
char *GetEntry(const char *list, int num)
{
  ...
  if (list[i] == ',') i++;   // skip the initial comma
  token[10] = 0;
  for (j = 0; j < 10; j++)
  {
    token[j] = list[i++];
