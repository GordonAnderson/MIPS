...
// is disabled. The following firmware updates are needed:
//    1.) Need to keep backwards compatibility. Use Rev = 2 to flag this mode.
...
int8_t BoardFromSelectedFilamentChannel(int8_t SC)
{
  ...
  // if selected channel is 0 or 1 then find the first available board and return it
  if (SC <= 1)
  {
    if (FilamentBoards[0]) return (0);
    if (FilamentBoards[1]) return (1);
...
//       by adjusting the filament supply voltage as needed. This minimizes the power dissipation in the
//       current source.
...
void Filament_loop(void)
{
  ...
  if(FLserialWD)
  {
    ...
    if (((FilamentBoards[0]) && ((FDarray[0].FCD[0].FilamentPwr) || (FDarray[0].FCD[1].FilamentPwr))) || ((FilamentBoards[1]) && ((FDarray[1].FCD[0].FilamentPwr) || (FDarray[1].FCD[1].FilamentPwr))))
    {
      ...
      if(FLserialTimer <= 0)
      {
        // If here we timed out so disable all the active boards in system
...
// Serial interface calibration functions. This is designed to calibrate a software rev 4.0
// filament system.
