...
//
// FAIMS fly back module commands
//
  ...
  for(int b = 0; b < 2; b++)
  {
    if(FAIMSFBarray[b] != NULL)
    {
     ...
     if(FAIMSFBrb[b] != NULL)
      ...
      if(!FAIMSFBarray[b]->Enable)
      {
        if(FAIMSFBstates[b]->Drive > 0)
        {
          TWIsetFloat(FAIMSFBarray[b]->TWIadd | 0x20, b, TWI_FB_SET_DRIVE, FAIMSFBstates[b]->Drive = 0);
        }
      }
      else if(FAIMSFBstates[b]->update || (FAIMSFBstates[b]->Drive != FAIMSFBarray[b]->Drive))
      {
        float drv;
        if(FAIMSFBstates[b]->Drive < FAIMSFBarray[b]->Drive)
        {
           drv = FAIMSFBstates[b]->Drive + 2;
           if(drv > FAIMSFBarray[b]->Drive) drv = FAIMSFBarray[b]->Drive;
        }
        else drv = FAIMSFBarray[b]->Drive;
        if(drv != FAIMSFBarray[b]->Drive) ramping = true;
        else ramping = false;
        TWIsetFloat(FAIMSFBarray[b]->TWIadd | 0x20, b, TWI_FB_SET_DRIVE, FAIMSFBstates[b]->Drive = drv);
      }
      ...
      if((!ramping) && (FAIMSFBarray[b]->Enable))
