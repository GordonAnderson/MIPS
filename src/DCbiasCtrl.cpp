...
// The board offset value allows the DCbias control mode to be applied to a DCbias
// module that board address is lower by this offset value. If the offest is set to
// 1 then DCbias module at board address 0 will use enable and use the DCbias control
// module. This value is automatically set when the DCbias control module is configured
// as and ADC.
// These features are used to allow ADC only usage.
int            brdOffset        = 0;
...
//
// DC bias control output module commands
//
...
// This function is called when the module is configured for ADC use only
void ADC_init(int8_t Board, int8_t addr)
{
  // If this board address 1 and a DCbias module is located at the same address
  // with a board address of 1 then set brdOffset tp 1 and exit. This is curently
  // the only mode.
  if(Board == 1)
  {
    if(DCbDarray[Board-1]!=NULL)
    {
      if(DCbDarray[Board-1]->EEPROMadr == addr)
      {
        brdOffset = 1;
        dcbctrlEnable(1);
      }
