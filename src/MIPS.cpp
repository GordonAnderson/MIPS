...
// are processed in the polling loop when there is available time. The serial command processor is table
...
// it scans all possible locations for EEPROMS looking for modules. These modules are identified by the name
// string. The MIPS system has a board select line and modules all have a A/B jumper to select there board
// address. Modules address are 50,52,54,56 (hex) and with board select this allows 8 modules maximum in one
// MIPS system.
//
...
const char Version[] PROGMEM = "Version 1.268" DCBswitchCodev DCBanalogv DCBcurrentv FAIMSFBvf FAIMSvf HOFAIMSvf HVPSv DMSDMSMBv HVPSinterfacev ",Sept 14,2026";
...
// USB error tracking (replaces custom USBCore.cpp additions)
static bool USBtimeout   = false;
static bool USBfatalError = false;
bool isUSBerror(void)    { return USBtimeout || USBfatalError; }
void clearUSBerror(void) { USBtimeout = USBfatalError = false; }
...
void setup()
{
  ...
  rtc.write(now.day(), now.month(), now.year(), now.hour(), now.minute(), now.second(), now.isPM(), false);
  digitalWrite(LT,LOW);
  //
...
void MIPSsystemLoop(void)
{
  ...
  // a warning on the display. Process serial commands and stay in this loop until
  ...
  if (ReadVin() < 10.0)
  {
    ...
    while (ReadVin() < 10.0)
    {
      // Process the power off command set!
      ReadAllSerial();
      if (RB_Commands(&RB)>0)
      {
        // Proces command in ring buffer
...
// This function will scan through all the possible addresses for the EEPROMs that are located on
...
void ScanHardware(void)
{
...
#if DCBanalog
      if (strcmp(&signature[2], "ADC") == 0) ADC_init(0, addr);
#endif
#if DMSDMSMB
...
#if DCBanalog
      if (strcmp(&signature[2], "ADC") == 0) ADC_init(1, addr);
#endif
#if DMSDMSMB
      if (strcmp(&signature[2], "CVBIAS") == 0) CVBIAS_init(0, addr);
      if (strcmp(&signature[2], "WAVEFORMS") == 0) WAVEFORMS_init(0, addr);
  ...
  HVPSinterface_init();
  #endif
  // The last thing we do is look for the analog input option
...
{
  #if DMSDMSMB
  // In case SPI is not initialized
  SPI.begin(SPI_CS);
  SPI.setClockDivider(SPI_CS, 8);  // Default divider is 21 and equals 4 MHz
  // Make sure LDAC IO lines are initialized
  LDACcapture;
  // Enable DIO
  Init_IOpins();
  ...
  SD.begin(_sdcs);
  SD.end();
  // Use DIO pin F on DMSDMS mother board
  SDIO_Set_Image('F', '1');
  DOrefresh;
  PulseLDAC;
  //UpdateDigitialOutputArray();
