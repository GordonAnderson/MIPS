...
//  1.250, December 20, 2024
//      1.) Added file IO functions:
//          - Create
//          - Add record to the file
//          - Read record number from file
//          - List a file's contents
//  1.251, December 23, 2024
//      1.) Updated the level change detection message to report the value to two places
//          after the decimal.
//      2.) Added support for ESI hardware rev 4.4
//  1.252, January 6, 2025
//      1.) Added OFF option to level change enable function
//      2.) Added window state report to level change module
//      3.) Added DIO change report hold off in mS
//  1.253, January 10, 2025
//      1.) Added the ability to remove a DCbias board from the group of common offset.
//          This is done my setting the modules UseOneOffset flag t0 2, used the memory
//          write function to set this value. (offset 124 hex in data structure)
//  1.254, January 21, 2025
//      1.) Fixed DCB readback bug introduced with 1.253
//  1.255, March 31, 2025
//      1.) Addressed a long standing bug that caused the DB bias system to stop updating the voltages
//          after a reported weeks to months of operation. Added a test in the thread library to prevents
//          negative time deltas from blocking a thread.
//      2.) added several additional thread management commands.
//  1.256, June 24, 2025
//      1.) Fixed a bug in the AD7884 TWI hardware mode driver
//  1.257, July 1, 2025
//      1.) Replaced RTC drive with softRTC to address the start up lock up issue.
//          I think this was due to RTC stability issues.
//  1.258, August 2, 2025
//      1.) Added analog record buffer function and commands, tied to delayed trigger function
//      2.) Fixed a bug in the memory allocation drivers for the DUE, requires libary mods
//          to implement.
//  1.259, August 20, 2025
//      1.) Added logical AND of two series compares to the P table loop counter compare functions
//      2.) Added ADC trigger and Trig out generation to P table mode
//      3.) Added ADC vector read sum function and command
//      4.) Added ADC vector read max value function and command
//      5.) Allow loading a table when in table mode if the tabel system is in READY mode. 
//          The new table structure cannot change, only the values can change, use with care
//  1.260, October 9, 2025
//      1.) Updated the RFamp driver. The update function and set resolving DC function did not perform
//          the update when the command was processed, only the values where set and updated when the task next
//          executed, the code was edited to cause the update to happen when the command is processed
//      2.) Added tune parameter adjustment command to RF quad system
//      3.) Added RF driver tune option to scan to max freq on initial step
//  1.261, December 14, 2025
//      1.) Added the acquire function to RF amplifier drivers, this supports faster m/z scanning.
//      2.) Updated the ADC sum function to wait for the acquire to finish.
//      3.) Updated TWITALK functions to use default TWI speed value.
//      4.) Colleced all TWI commands into one section in serial.
//      5.) Added GTWI and STWI to support generic TWI IO to any device.
//      6.) Added the High Voltage power supply interface.
//      7.) Fixed a long standing but in the GetLine function, the line pointer was never initialized
//      8.) Updated code to support power cycling using the F line for the DMSDMSMB version
//      9.) Added command processing when off and support minimul commands
//      10.) Added the L option to the toggle DC bias profiles option. This uses the level sensor to
//           trigger a toggle ans the width determins the applided profile.
//      11.) Added ramping when setting the drive level for FAIMSfb
//  1.262, Feb 7, 2026
//      1.) Updated ESI driver to support 4 modules and dynamically allocate its data structures.
//      2.) Update the RFdriver module to support both models of RFdriver boards.
//      3.) Removed the table2 switch and now permanently use table2, the old table code has been removed.
//      4.) Updated the trigger out (t) signal in the timing generator. t was desiged to generate a pulse
