 ...
 *
 * Updated to disable the PLL before update. This is recommended in the data sheet to
...
int CY_Init(int8_t adr, int8_t brd)
{
  int iStat;
  ...
  //
  CYregs[brd].RegAdd = 0x08;
  CYregs[brd].Osc = 0b10010101;		// Crystal, 7PF, 30 ohms
  // Set clock A and C to PLL2
  //   Clk*_fs[2:0] = 100
  CYregs[brd].ClkA_Div_DS0 = 0b00000000;
  CYregs[brd].ClkA_Div_DS1 = 0b00000000;
  CYregs[brd].ClkC_Div = 0b00000000;
  // Set clock B and D to PLL3
  //   Clk*_fs[2:0] = 110
  CYregs[brd].ClkB_Div_DS0 = 0b00000000;
  CYregs[brd].ClkB_Div_DS1 = 0b00000000;
  CYregs[brd].ClkD_Div = 0b00000000;
  CYregs[brd].ClkABCD_FS = 0b11101110;	// FS[2:1]
  // Set output duty cycle, enables, clkE_div
  CYregs[brd].Clk_ACadj = 0b01010101;
  // Set the output drive level
  CYregs[brd].Clk_DCadj = 0b01010101;
  // PLL2
  CYregs[brd].PLL2_Q = 0;
  CYregs[brd].PLL2_P = 0;
  CYregs[brd].PLL2_Misc = 0b01000000;
  // PLL3
  CYregs[brd].PLL3_Q = 0;
  CYregs[brd].PLL3_P = 0;
  CYregs[brd].PLL3_Misc = 0b01000000;
  //
...
void SetPLL2enable(int8_t adr, bool state, int8_t brd)
{
  if(state)
  {
     Wire.beginTransmission(adr);
     Wire.write(0x08);
     Wire.write(CYregs[brd].ClkA_Div_DS0);
     Wire.write(CYregs[brd].ClkA_Div_DS1);
...
int SetPLL2freq(int8_t adr, int Freq, int8_t brd)
{
  int iStat;
  ...
  CYregs[brd].PLL2_P = P & 0xFF;
  CYregs[brd].PLL2_Misc = (CYregs[brd].PLL2_Misc & 0xC0) | (LF << 3) | (P0 << 2) | (P >> 8);
  CYregs[brd].PLL2_Q = Q;
  CYregs[brd].ClkA_Div_DS0 = (CYregs[brd].ClkA_Div_DS0 & 0x80) | (Div & 0x7F);
  CYregs[brd].ClkA_Div_DS1 = (CYregs[brd].ClkA_Div_DS1 & 0x80) | (Div & 0x7F);
  CYregs[brd].ClkC_Div = (CYregs[brd].ClkC_Div & 0x80) | (Div & 0x7F);
  // Disable PLL2
  CYregs[brd].PLL2_Misc &= ~0x40;
  Wire.beginTransmission(adr);
  Wire.write(0x13);
  Wire.write(CYregs[brd].PLL2_Misc);
  Wire.endTransmission();
  //
...
// If state = true then PLL2 will be enabled, else disabled.
void SetPLL3enable(int8_t adr, bool state, int8_t brd)
{
  if(state)
  {
     Wire.beginTransmission(adr);
     Wire.write(0x0A);
     Wire.write(CYregs[brd].ClkB_Div_DS0);
     Wire.write(CYregs[brd].ClkB_Div_DS1);
     Wire.endTransmission();
  }
  else
  {
...
int SetPLL3freq(int8_t adr, int Freq, int8_t brd)
{
  int iStat;
  ...
  //
  FindPQ(Freq);			// Determine the PLL values
  CYregs[brd].PLL3_P = P & 0xFF;
  CYregs[brd].PLL3_Misc = (CYregs[brd].PLL3_Misc & 0xC0) | (LF << 3) | (P0 << 2) | (P >> 8);
  CYregs[brd].PLL3_Q = Q;
  CYregs[brd].ClkB_Div_DS0 = (CYregs[brd].ClkB_Div_DS0 & 0x80) | (Div & 0x7F);
  CYregs[brd].ClkB_Div_DS1 = (CYregs[brd].ClkB_Div_DS1 & 0x80) | (Div & 0x7F);
  CYregs[brd].ClkD_Div = (CYregs[brd].ClkD_Div & 0x80) | (Div & 0x7F);
  // Disable PLL3
  CYregs[brd].PLL3_Misc &= ~0x40;
  Wire.beginTransmission(adr);
  Wire.write(0x16);
  Wire.write(CYregs[brd].PLL3_Misc);
  Wire.endTransmission();
  //
  // Send to the device using TWI
  //
...
int FAIMSclockSet(int8_t adr, int Freq)
{
  ...
  Wire.beginTransmission(adr);
  Wire.write(0x13);
  Wire.write(CYregs[0].PLL2_Misc);
  Wire.endTransmission();
  CYregs[0].RegAdd = 0x08;
  CYregs[0].Osc = 0b10010101;    // Crystal, 7PF, 30 ohms
  // Set the dividers
  CYregs[0].ClkA_Div_DS0 = 24;
  CYregs[0].ClkA_Div_DS1 = 24;
  CYregs[0].ClkB_Div_DS0 = 120;
  CYregs[0].ClkB_Div_DS1 = 120;
  CYregs[0].ClkC_Div = 60;
  CYregs[0].ClkD_Div = 30;
  CYregs[0].Clk_ACadj = 1;  // Set E out divider to 4
  // Set the clock sources to PLL1
  CYregs[0].ClkABCD_FS = 0b10101010;
  // Set output duty cycle, enables, clkE_div
  CYregs[0].Clk_ACadj |= 0b01010100;
  // Set the output drive level
  CYregs[0].Clk_DCadj = 0b00000000;
  ...
  CYregs[0].PLL2_P = P & 0xFF;
  CYregs[0].PLL2_Misc = (0x00) | (LF << 3) | (PO << 2) | (P >> 8);
  CYregs[0].PLL2_Q = Q;
  CYregs[0].PLL3_P = P & 0xFF;
  CYregs[0].PLL3_Misc = (0x00) | (LF << 3) | (PO << 2) | (P >> 8);
  CYregs[0].PLL3_Q = Q;
