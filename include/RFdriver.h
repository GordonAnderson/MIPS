...
typedef struct  // 147 bytes
{
  ...
  // These parameters really belong in RFchannelData but there were added after release so placing at end of struct gives backwards compatibility
  ...
  int     dummy;                 // needed to make the two structure the same size
} RFdriverData;
...
extern   RFdriverData  *rfddarray[MAXRF];
extern   float         gatedDrive[MAXRF];
...
#define TWI_RF_READ_AVALIBLE      0x82      // Returns the number of bytes available in output buffer, 16 bit unsigned int
