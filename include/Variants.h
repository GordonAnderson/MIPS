...
//
// This file contains all the possible board data structures and their default values. This allows a user
...
#define    TMR_Profiles       Timer5  // Used for the profile toggling function
...
// MIPS system level configuration data structure
typedef struct
{
  int16_t  Size;
  char     Name[20];          // Holds the MIPS box name
  int      Rev;               // Holds the hardware rev level
  uint8_t  DOlsb;             // Digital output image registers, LSB
  uint8_t  DOmsb;             // Digital output image registers, MSB
  ...
  bool     DisableDisplay;    // Save the disable display state
