...
#define TWI_ARB_SET_ENABLE      0x07      // Enables or disables the ARB outputs. 0 = disable, 1 = enables
...
#define TWI_ARB_SET_ARB_VECTOR  0x13      // Set ARB vector in buffer, format:
                                          ...
                                          // 8 bit unsigned values. 0 to 255. The index will automatically advance to allow long buffer fills.
...
#define TWI_SET_SINE            0x39      // Define one sine wave cycle for the selected channel and defined starting phase
                                          // This command is for conventional ARB mode, channel 0 to 7 (byte), phase is a float in degrees
...
typedef struct
{
  ...
  int8_t        NumChannels;       // Number of channels supported by this board
  ...
  uint8_t       EEPROMadr;
  // Additional parameters added for rev 2 upgrade to ARB module
  ...
  char          Cswitch;            // Digital output to control output switch to reload ions to mass spec
...
extern ARBdata  arb;
extern DialogBox ARBdialog;
extern DialogBoxEntry ARBentriesPage1[];
...
extern int NumberOfARBchannels;
...
// Low-level TWI communication helpers
