...
#define TWI_FB_SET_EXTSTEP     0x11      // Set to true to enable external step advance, bool
...
#define TWI_FB_SET_FLUSH       0x15      // Flushes the output buffer
...
#define TWI_FB_SET_VRF_NOW     0x18      // Set the Vrf voltage setpoint and then adjust the drive to achieve the setpoint
...
#define TWI_FB_READ_AVALIBLE   0x82      // Returns the number of bytes available in output buffer, 16 bit unsigned int
...
typedef struct  // 260 byte
{
  ...
  // The following variables were added June 3, 2021. These support the new
