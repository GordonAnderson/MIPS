...
// Include display library before defining pin macros to avoid name conflicts
// with the library's parameter names (_cs, _dc, _rst, etc.)
#include "SPI.h"
...
// Digital input pins, Q through X
#define  DI0          30
#define  DI1          12
#define  DI2          50
#define  DI3          51
...
// Level detection module constants. This module can be installed in MIPS using the aux connector
...
#define TWI_LEVDET_READ_AVALIBLE   0x82      // Returns the number of bytes available in output buffer, 16 bit unsigned int
...
#define TWI_LEVDET_READ_WSTATE     0x85      // Returns window mode state, 1=Above,2=Within,3=Below, byte value
...
int Value2Counts(float Value, DACchan *DC, float gc = 1.0, int limit = 65535);
int Value2Counts(float Value, ADCchan *AC, float gc = 1.0, int limit = 65535);
